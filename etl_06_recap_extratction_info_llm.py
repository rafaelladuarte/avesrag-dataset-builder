...
from prompts.morphology_llm import formatted_color_prompt
...
if __name__ == "__main__":
    ...
    path_json = "data/oficial/oficial_dataset_udi.json"
    ...
    for i, bird in enumerate(list_birds):
        try:
            ...
            if bird.get("cores2", None) is None:

                prompt_hab = formatted_color_prompt(
                    order, family, genre,
                    specie, resume
                )
                ...
                habitat_llm = string_to_dict(return_hab_llm)
                print(habitat_llm)
                bird["cores2"] = habitat_llm.get("cores", None)
                print("- Get information habitat for bird")
